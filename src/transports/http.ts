 ...
 * Sets up a proxy hook on pendingAuths.set to automatically start polling for new relay sessions.
 */
function setupRelayPolling(
  relayBaseUrl: string,
 ...
 * Handles session management, user isolation, and Streamable HTTP transport.
 */
function registerMcpRoutes(
  app: express.Express,
  mcpRateLimit: express.RequestHandler,
  authMiddleware: express.RequestHandler,
  _provider: OAuthServerProvider,
 ...
 */
function registerHealthCheck(app: express.Express, userAccounts: Map<string, AccountConfig[]>) {
...
export async function startHttp(): Promise<void> {
  ...
  // Setup background relay polling
  setupRelayPolling(relayBaseUrl, pendingAuths, userAccounts, authCodes)
  ...
  // Register MCP routes
  registerMcpRoutes(app, mcpRateLimit, authMiddleware, provider, resolveAccounts)
