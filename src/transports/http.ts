...
/** Partition accounts into Outlook/OAuth2 and standard IMAP buckets. */
...
/** Save accounts to per-user store (if sub exists) and shared config.enc fallback. */
async function saveAndPersistCredentials(
  raw: string,
  ...
  if (sub) {
    try {
      ...
      throw new Error('Failed to save credentials. Please retry.')
    }
...
/** Handle credential saving, validation, and OAuth initiation. */
async function handleOnCredentialsSaved(
  ...
  // Ensure fresh device-code flow by resetting token cache
  _resetTokenCache()
  ...
  try {
    await saveAndPersistCredentials(raw, accounts, context?.sub)
  } catch (err) {
...
function buildOptions(args: {
  ...
  return {
    ...
    port,
    host,
    onCredentialsSaved: (creds, context) => handleOnCredentialsSaved(creds, context, onAccountsLoaded),
