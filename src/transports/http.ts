...
/**
 * Persist credentials to the appropriate store (per-user or global).
 */
...
/**
 * Handle saved credentials from the UI form.
 */
async function handleCredentialsSaved(args: {
  ...
  const persistResult = await persistCredentials({ raw, accounts, sub, onAccountsLoaded })
...
function buildOptions(args: {
  ...
  return {
    ...
    port,
    host,
    onCredentialsSaved: (creds, context) => handleCredentialsSaved({ creds, context, onAccountsLoaded }),
