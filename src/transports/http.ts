...
/** Categorize accounts into Outlook/OAuth2 and IMAP groups. */
...
/** Handle both per-user and shared credential persistence. */
...
/** Orchestrate the full credential saving flow. */
async function handleOnCredentialsSaved(
  ...
  const saveResult = await saveAndPersistCredentials(raw, accounts, context)
