  ...
  return crypto.subtle.deriveKey(
    {
      name: 'PBKDF2',
      hash: 'SHA-256',
      salt: encoder.encode('mcp-email-per-user'),
      iterations: 100_000
    },
    keyMaterial,
...
export async function loadAllUserCredentials(): Promise<Map<string, AccountConfig[]>> {
  ...
  let entries: any[]
  try {
    entries = await readdir(_paths.DATA_DIR, { withFileTypes: true })
  } catch (err: any) {
  ...
  await Promise.all(
    entries.map(async (entry: any) => {
      ...
      const credPath = join(_paths.DATA_DIR, String(entry.name), 'credentials.enc')
      try {
        const data = await readFile(credPath)
        const iv = data.subarray(0, 12)
        const ciphertext = data.subarray(12)
