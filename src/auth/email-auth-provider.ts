 ...
 * Email Auth Provider
 *
 * Implements a multi-user email-based OAuth provider for the MCP server.
 * This provider handles the "Auth Relay" flow where credentials (IMAP/SMTP)
 * are submitted via a secure relay service and stored in-memory (or persisted)
 * to be used for future tool calls.
 *
...
function createProvider(
  ...
  clientStore: StatelessClientStore,
  pendingAuths: Map<string, PendingAuth>,
  authCodes: Map<string, StoredAuthCode>,
  bearerTokens: Map<string, StoredToken>,
  _boundTokens: Map<string, StoredToken>,
  userAccounts: Map<string, AccountConfig[]>,
  verifyCache: Map<string, { expiresAt: number; userId: string }>,
  pendingBinds: Map<string, { token: StoredToken; expiresAt: number; sourceIp?: string }>,
  resolveUserId: (token: string) => string | undefined
): OAuthServerProvider {
  return {
