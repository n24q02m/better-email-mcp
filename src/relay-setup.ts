...
/**
 * Handle post-setup tasks: OAuth verification for Outlook accounts and relay messaging.
 */
async function handlePostRelaySetup(
  ...
  const sessionUrl = `${relayUrl}/api/sessions/${session.sessionId}/messages`
...
/**
 * Run interactive relay setup: create session, poll for result, and save config.
 *
 * Returns the raw config object or null if setup failed/skipped.
 */
async function triggerRelaySetup(relayUrl: string): Promise<Record<string, string> | null> {
  ...
  try {
    ...
    // Handle post-setup tasks (OAuth, messaging)
    await handlePostRelaySetup(relayUrl, session, config)
...
export async function ensureConfig(): Promise<string | null> {
  ...
  const config = await triggerRelaySetup(DEFAULT_RELAY_URL)
  return config ? formatCredentials(config) : null
}
