...
// ⚡ Bolt: Cache bigrams and lowercase values for static valid options to avoid recomputing them.
// Keyed by the original option string to avoid even the toLowerCase() call on cache hits.
...
export function findClosestMatch(input: string, validOptions: string[]): string | null {
  ...
  for (const option of validOptions) {
    // ⚡ Bolt: Use cached bigrams and lowercase value if available, otherwise compute and cache them.
    // Keying by 'option' (original string) avoids redundant toLowerCase() calls.
