...
// ⚡ Bolt: Cache bigrams and lowercase strings for static valid options.
// Keying by the original option string ensures we only compute lowercase strings and bigrams once per static option.
...
export function findClosestMatch(input: string, validOptions: string[]): string | null {
  ...
  for (const option of validOptions) {
    // ⚡ Bolt: Fetch or compute the cached state for this option.
