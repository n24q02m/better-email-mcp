...
// ⚡ Bolt: Cache bigrams and lowercase values for static valid options to avoid recomputing them on every request.
// This minimizes map lookups and prevents redundant string operations.
...
export function findClosestMatch(input: string, validOptions: string[]): string | null {
  ...
  for (const option of validOptions) {
    // ⚡ Bolt: Key cache by the original option string and cache derived states.
    ...
    if (!cached) {
      ...
      cached = { lower: optionLower, bigrams: optionBigrams }
    ...
    let overlap = 0
    // ⚡ Bolt: Iterate over the smaller set to calculate the intersection/overlap for maximum performance.
    const [smallerSet, largerSet] =
      inputBigrams.size < optionBigrams.size ? [inputBigrams, optionBigrams] : [optionBigrams, inputBigrams]

    for (const b of smallerSet) {
      if (largerSet.has(b)) overlap++
    }
