...
// ⚡ Bolt: Pre-compile snippet parsing regexes into module constants.
// This eliminates redundant RegExp object allocation when `fastExtractSnippet`
// is called repeatedly during high-volume array mapping operations.
const WHITESPACE_REGEX = /\s+/g
const STYLE_SCRIPT_TEST_REGEX = /<(?:style|script)/i
const STYLE_SCRIPT_REPLACE_REGEX = /<(style|script)\b[^>]*>[\s\S]*?(?:<\/\1\s*>|$)/gi
const BLOCK_ELEMENT_REPLACE_REGEX = /<\/(p|div|br|tr|li|h[1-6])>/gi
const BR_TAG_REPLACE_REGEX = /<br\s*\/?>/gi
const ANY_TAG_REPLACE_REGEX = /<[^>]+>/g
const ENTITY_DECODE_REGEX = /&(#(?:x|X)?[\da-fA-F]+|[a-zA-Z]+);/g
...
export function fastExtractSnippet(html: string, maxLength = 200): string {
  ...
  text = text.replace(BLOCK_ELEMENT_REPLACE_REGEX, ' ')
  text = text.replace(BR_TAG_REPLACE_REGEX, ' ')

  // Strip all remaining HTML tags
  text = text.replace(ANY_TAG_REPLACE_REGEX, '')
  ...
  if (text.indexOf('&') !== -1) {
    text = text.replace(ENTITY_DECODE_REGEX, (entity, p1) => {
