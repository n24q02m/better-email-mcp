...
function isPortSegment(s: string) {
  return /^\d+$/.test(s)
}
...
function extractSmtpSegments(tail: string[]): { host?: string; port?: number; security?: SmtpSecurity } {
...
function parsePasswordAndHost(parts: string[]): {
  ...
  const { host: smtpHost, port: smtpPort, security: smtpSecurity } = extractSmtpSegments(tail)
  const { host: imapHost, port: imapPort } = extractImapSegments(tail)
