...
async function handleArchive(accounts: AccountConfig[], input: MessagesInput): Promise<any> {
  ...
  // Check cache first
  let cachedArchiveFolder = archiveFolderCache.get(account.id)
  let defaultArchiveFolder: string | undefined

  if (!cachedArchiveFolder) {
    // Detect archive folder based on provider
    defaultArchiveFolder = '[Gmail]/All Mail'
  ...
  const result = await archiveEmails(account, uids, folder, cachedArchiveFolder || defaultArchiveFolder)

  // Update cache if we found it
  if (!cachedArchiveFolder && result.archiveFolder) {
