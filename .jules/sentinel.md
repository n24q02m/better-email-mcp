...
**Vulnerability:** The Cloudflare worker in `src/worker.ts` adds security headers (HSTS, X-Content-Type-Options, X-Frame-Options) only to the responses successfully proxied by the public `fetch` entrypoint. However, the internal `kvOutbound` handler (which answers outbound HTTP requests to `kv.internal` via the ContainerProxy) did not apply these headers to its responses.
**Learning:** In edge routing patterns where internal handlers are exposed via specific binding endpoints (like Cloudflare's outbound proxying), missing security headers on these endpoints fail defense-in-depth reviews. If an SSRF vulnerability was present elsewhere, these handlers could be abused without basic HTTP protections.
**Prevention:** Implement a global response wrapper function (e.g., `withSecurityHeaders(response: Response)`) that clones and sets standard security headers on *every* outbound response originating from the worker, including those from internal handlers like `kvOutbound`.
