...
## 2026-07-23 - [Unvalidated OAuth 2.0 Device Code Response Headers]
**Vulnerability:** The `requestDeviceCode` function in `src/tools/helpers/oauth2.ts` performed an HTTP `fetch` to Microsoft's OAuth 2.0 endpoint but did not validate the `Content-Type` of the response before parsing it as JSON. If the endpoint returned HTML (e.g., due to a proxy error or captive portal), it would throw an unexpected `SyntaxError` which might not be handled gracefully by the polling mechanism or might expose internal states.
**Learning:** Always validate the `Content-Type` header of external API responses before blindly parsing the body using `response.json()`.
**Prevention:** Check that the `Content-Type` header includes `application/json` before attempting to parse the response body as JSON. If it does not, throw a descriptive error or handle it as an invalid response.
