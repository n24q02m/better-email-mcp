...
## 2025-02-28 - Sanitize window.location.replace and remove innerHTML usage
**Vulnerability:** A `window.location.replace(pendingRedirectUrl)` call in `src/credential-form.ts` was used to redirect users without validating the protocol. Also, `innerHTML` was being used to set button content, which could potentially lead to XSS if not carefully controlled, despite the current inputs being statically defined strings.
**Learning:** Even when reading URLs from a trusted backend, client-side code should treat the values as untrusted inputs and validate protocols before using them for redirects. Also, strictly adhere to DOM methods like `createElement` and `appendChild` over `innerHTML`.
**Prevention:** Always use a safe protocol parsing mechanism like `new URL()` to check `.protocol === "http:" || .protocol === "https:"` before feeding dynamic URLs to `window.location.assign`, `window.location.replace`, or assigning them to `location.href`. Avoid using `innerHTML` and use `createElement` and `textContent` instead.
