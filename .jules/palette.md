...
## 2024-06-11 - Semantic structure and ARIA in credential form
**Learning:** The credential form dynamically generates elements like status indicators for Outlook authorization without explicitly declaring them as live regions (`aria-live="polite"`). Adding `aria-live` attribute natively enables screen readers to announce "Waiting for Microsoft authorization..." updates. Also, linking `<form>` with its title using `aria-labelledby="form-title"` enforces clear bounds for the complex, multi-account setup UI.
**Action:** When creating dynamic loading or waiting indicators with `createElement`, remember to use `setAttribute("aria-live", "polite")`. Tie form blocks to their context headers via `aria-labelledby` for robust screen reader navigation.
