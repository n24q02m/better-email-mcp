...
## 2024-03-24 - Smooth Programmatic Focus and Tooltip Enhancements
**Learning:** While `aria-label` is great for screen readers, icon-only or generic-label buttons (like "Remove" or password visibility toggles) often leave sighted users guessing about their exact target or function (e.g., "Remove Account 2"). Furthermore, standard `.focus()` calls on dynamically inserted or removed form elements can cause jarring visual jumps, confusing both sighted and screen-reader users as to where they are on the page.
**Action:** Always complement `aria-label` with native `title` attributes on icon/action buttons to provide visible tooltip hints for sighted users. When programmatically focusing elements (e.g., moving focus after adding/removing a field or showing validation errors), use `.focus({ preventScroll: true })` followed by `.scrollIntoView({ behavior: 'smooth', block: 'nearest' })` to ensure a smooth, predictable transition that keeps the user oriented.
