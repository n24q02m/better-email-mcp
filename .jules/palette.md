...
## 2026-06-05 - Focus management in novalidate forms
**Learning:** Forms that use the `novalidate` attribute skip the browser's default validation behavior, which means they don't automatically focus the first invalid field or show default tooltips upon submission. If we prevent default submission entirely, keyboard and screen reader users can be left stranded without knowing why submission failed, as the focus is lost or kept on the submit button.
**Action:** Always call `form.checkValidity()` in the custom submit handler for `novalidate` forms. If validation fails, manually query for `input:invalid, select:invalid, textarea:invalid`, check if the element has a `.focus()` method, and call it to guide the user to the exact field that requires correction.
