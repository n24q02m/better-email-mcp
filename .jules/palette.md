...
## 2025-05-26 - Focus Management for Native Form Validation
**Learning:** Forms utilizing the `novalidate` attribute require manual implementation of focus management upon failed submission, as native browser validation popups and automatic focusing are disabled.
**Action:** When implementing custom form validation on forms with `novalidate`, explicitly call `form.checkValidity()` on submit. If invalid, programmatically find the first invalid element using `form.querySelector('input:invalid, select:invalid, textarea:invalid')` and call `.focus()` to ensure accessibility for keyboard users and screen readers. Note that when queried like this, ensure it is verified with `instanceof HTMLElement` before `.focus()` is called.
