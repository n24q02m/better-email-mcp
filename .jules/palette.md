...
## 2025-06-15 - Explicit Form Field Guidance and Constraints
**Learning:** Depending entirely on external 'help text' to explain expected inputs places unnecessary cognitive load on users. They must scan away from the input field to understand constraints. Additionally, lack of immediate input constraint (e.g., typing letters into a port field) leads to frustrating errors only visible post-submission.
**Action:** Always provide explicit, inline guidance within form schema declarations. Use `placeholder` to provide concrete, visually proximate examples (e.g., `imap.example.com`). Utilize `validation` rules (e.g., regex pattern `^\d+$` for a port) to enable immediate client-side feedback and prevent invalid character entry before the form is even submitted. This creates a much more intuitive and guided user experience.
