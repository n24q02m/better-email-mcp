...
## 2024-05-19 - Form Validation Focus on Novalidate Forms
**Learning:** In forms marked with `novalidate` (common for custom error UIs), the standard HTML5 validation bubble is suppressed, but the underlying `ValidityState` and `invalid` events still work. When intercepting submit and checking `.checkValidity()`, focus is NOT automatically managed by the browser. Screen reader and keyboard users can easily get lost or miss errors if focus stays on the submit button.
**Action:** When overriding form submission logic on a `novalidate` form with `form.checkValidity()`, always programmatically find the first invalid element (using `form.querySelector('input:invalid, select:invalid, textarea:invalid')`), check it `instanceof HTMLElement`, and call `.focus()`.
