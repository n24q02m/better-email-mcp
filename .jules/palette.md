...
## 2026-05-15 - [Form Input Placeholders & Native Validation]
**Learning:** Empty password fields for App Passwords consistently lead to users typing their regular account passwords, causing silent setup failures. Adding concrete, instructional placeholders (e.g., '16-digit App Password') directly into the input fields reduces friction. Also, using the native HTML5 validation system via `pattern` attribute matching (e.g., `validation: '^\d+$'`) provides immediate inline feedback, which significantly improves usability before the form hits the backend.
**Action:** When designing credential forms requiring special auth tokens or specific formats, always inject clear hint placeholders and lean on the browser's native `pattern` capabilities for immediate client-side validation to catch simple errors (like non-numeric port numbers) early.
