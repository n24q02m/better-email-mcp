...
## 2025-05-31 - Bigram Set Intersection Performance Optimization
**Learning:** In string similarity algorithms based on bigrams (like Sorensen-Dice), checking intersection using an arbitrary set can lead to unnecessary iterations and map lookups.
**Action:** When calculating set intersections, always determine the smaller set first and iterate over it while calling `largerSet.has(item)`. Additionally, consolidate state caching (e.g. `lower` and `bigrams`) into a single object indexed by the original string to avoid redundant allocations.
