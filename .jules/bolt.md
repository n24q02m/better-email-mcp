...
## 2025-05-18 - [Eliminate Object.assign reallocation in hot path loops]
**Learning:** In hot paths and tight loops (e.g., query parsing in `buildSearchCriteria`), using `Object.assign(obj, { [key]: value })` with an inline object literal dynamically allocates a new object on every invocation before assigning properties. This causes unnecessary intermediate object allocations and increases garbage collection overhead.
**Action:** Prefer direct property assignment (`obj[key] = value`) to prevent these intermediate allocations. If TypeScript interfaces prevent safe dynamic indexing (e.g., throwing TS2322), type the accumulator as `Record<string, any>` internally rather than the strict interface, and cast back at the return site.
