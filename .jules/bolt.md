...
## 2026-05-11 - Signal Outlook OAuth completion to frontend form
 **Learning:** The frontend `credential-form.ts` polls `GET /setup-status` and expects `s.outlook === "complete"` to stop the "Waiting for Microsoft authorization..." spinner. However, the background device-code poll only updated the internal `state` to `configured` but never signaled completion to the `mark_setup_complete` hook.
 **Action:** Updated `src/transports/http.ts` to call `getMarkSetupComplete()?.('outlook')` in the `onComplete` callback of `initiateOutlookDeviceCode`. This correctly transitions the frontend state and provides a better user experience by removing the infinite spinner.
