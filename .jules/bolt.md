...
## 2024-08-24 - V8 Object Allocation Overhead in Tight Loops
**Learning:** In JavaScript V8 environments (like Node.js/Bun), using `Object.assign(obj, { [key]: value })` inside tight loops (like text parsing) allocates unnecessary intermediate objects on every iteration. While concise, this introduces significant garbage collection pressure and memory overhead in hot paths.
**Action:** When building up objects iteratively in frequently executed code, always prefer direct property assignment (`obj[key] = value`) over `Object.assign`. If strict TypeScript interfaces block dynamic indexing (e.g., throwing TS2322), define the accumulator as `Record<string, any>` instead of the strict interface, and optionally cast the final result.
