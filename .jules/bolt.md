...
## 2026-10-27 - [Safe global RegExp hoisting]
**Learning:** Extracting global regular expressions (those with the `g` flag) to module-scoped constants could potentially lead to bugs due to the leakage of the stateful `lastIndex` property across calls. However, they are completely safe from state leakage when used exclusively with `String.prototype.replace()`, as this method automatically resets `lastIndex` to 0 before execution.
**Action:** When extracting global regular expressions to avoid recompilation overhead in hot paths, verify that they are only used with `String.prototype.replace()`. If they are used with `RegExp.prototype.test()` or `RegExp.prototype.exec()`, they must either not use the `g` flag, or `lastIndex` must be manually reset.
