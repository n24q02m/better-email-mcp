...
## 2024-08-05 - [Avoid Regex Backreferences in Hot Paths]
**Learning:** In V8 environments (Node.js/Bun), regular expressions containing backreferences (e.g. `\1`) cannot be optimized and execute significantly slower (up to 10x slower). When stripping symmetric tags (like `<style>...</style>` and `<script>...</script>`) in high-frequency functions (like `fastExtractSnippet`), using a single regex with a backreference to match both tag types introduces a major performance bottleneck.
**Action:** When parsing symmetric HTML elements in hot paths, avoid backreferences. Instead, combine the specific patterns (e.g. one for style, one for script) using the OR operator (`|`). This preserves the correct left-to-right parsing semantics while allowing V8 to use fast-path execution.
