...
## 2024-08-21 - buildSearchCriteria property assignment
**Learning:** `Object.assign` in hot paths like query parsing loops allocates unnecessary intermediate objects, leading to garbage collection overhead. Using `Object.assign(criteria, { [key]: value })` allocates a new literal object for every assignment.
**Action:** Use direct property assignment (`criteria[key] = value`) combined with typing the accumulator as `Record<string, any>` rather than a strict interface if the TS interface prevents safe indexing.
