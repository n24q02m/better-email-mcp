...
## 2026-05-29 - [Optimize string allocation in bigram caching]
**Learning:** In `findClosestMatch`, the cached bigrams were not associated with the original string but with the `.toLowerCase()` version. The function still performed `.toLowerCase()` on every valid option during every match attempt, resulting in redundant string allocations and iteration over all options.
**Action:** When caching derived data (like bigrams or lowercase conversions) from static sets, ensure the cache map is keyed by the *original* string and that both the derived format (e.g., lowercased text) and the structural data (e.g., Set of bigrams) are stored within a single object value in the map cache to completely bypass redundant calculations.
