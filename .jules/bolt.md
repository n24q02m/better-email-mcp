...
## 2026-05-28 - [Set Intersection Optimization and Compound Caching]
**Learning:** In set-based similarity algorithms (like Sorensen-Dice), iterating over the first set to compute intersection with `.has()` on the second set is suboptimal when sizes vary. Iterating over the smaller set minimizes lookups. Additionally, caching only one derived state (like bigrams) leaves other invariants (like lowercase strings) to be recomputed.
**Action:** When calculating set intersections, always determine the smaller set and iterate over it. When caching derived data for a single entity, group all expensive derived states (e.g., lowercased string AND bigram set) into a single cache object to eliminate multiple map lookups and redundant processing.
